...
import { convertWebpToJpg } from '@/lib/image/processor';
...
export async function POST(req: NextRequest) {
  try {
    ...
    // 1. Validation - only WebP accepted
    const validation = await validateImageBuffer(inputBuffer, ['image/webp']);
    if (!validation.valid || !validation.width || !validation.height) {
      return apiError(validation.error || 'Please provide a valid WebP image.');
    }
    ...
    let result;
    try {
      result = await convertWebpToJpg(inputBuffer, { quality });
    } finally {
      release();
    }
    ...
    return apiError('An error occurred while converting your WebP to JPG. Please try again.');
  }
}
